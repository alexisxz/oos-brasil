...
export default function Home() {
  ...
  const stateOptions = [
    { name: "Todos", value: "" },
    ...states.map((s) => ({ name: s, value: s })),
  ];
  const citiesOptions = [
    { name: "Todos", value: "" },
    ...cities.map((s) => ({ name: s, value: s })),
  ];
  const organizationOptions = [
    { name: "Todos", value: "" },
    ...organizationsName.map((s) => ({ name: s, value: s })),
  ];
  const typeOptions = [
    { name: "Todos", value: "" },
    ...types.map((s) => ({ name: s, value: s })),
  ];
  ...
  const currentOrganization = organizations.slice(
  ...
  useEffect(() => {
    ...
    if (
      Object.values(filters).filter((filterValue) => !!filterValue).length === 0
    )
    ...
    Object.keys(filters).forEach((filterKey) => {
      ...
      newOrganizationsArray = newOrganizationsArray.filter(
        (organization) => (organization as any)[filterKey] === valueToBeFiltered
      );
    });
  ...
  const readDataFirebase = async () => {
    await getDocs(databaseRef).then((response) => {
      setGetOrganizations(
        response.docs.map((organization) => {
  ...
  const getOrganizationsName = () => {
    ...
    if (filters.state === "" && filters.city === "" && filters.type === "") {
      getOrganizations.map((organization) => {
        if (
          allOrganizationsName.find(
            (organizationName) => organizationName === organization.organization
          )
        )
          return;
        return (allOrganizationsName = [
          ...allOrganizationsName,
  ...
  const handleOnChange = (
    filter: string,
    selected: SelectedOptionValue | SelectedOptionValue[]
  ) => {
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          name="description"
          content="Se organize em uma organização socialista perto de você"
        />
        ...
        <main className={styles.main}>
          <div className={styles.mainDiv}>
            <p style={{ textAlign: "center" }}>
              Procure e organize-se o mais próximo de você{" "}
            </p>
            <div className={styles.organizationsFilters}>
              ...
              <SelectSearch
                ...
                search={true}
                placeholder="Estado"
                onChange={(s) => handleOnChange("state", s)}
                value={filters.state}
              />
            </div>
            ...
            <div className={styles.organizationsFilters}>
              ...
              <SelectSearch
                ...
                search={true}
                placeholder="Cidade"
                onChange={(s) => handleOnChange("city", s)}
                value={filters.city}
              />
            </div>
            ...
            <div className={styles.organizationsFilters}>
              ...
              <SelectSearch
                ...
                onChange={(s) => handleOnChange("organization", s)}
            ...
            <div className={styles.organizationsFilters}>
              ...
              <SelectSearch
                options={typeOptions}
                search={true}
                placeholder="Tipe de organização"
                onChange={(s) => handleOnChange("type", s)}
                value={filters.type}
              />
            </div>
          ...
          <div className={styles.mainDiv}>
            ...
            {getOrganizations.length <= 0 ? (
              <p style={{ textAlign: "center" }}>Carregando organizações...</p>
            ) : organizations.length <= 0 ? (
              <p style={{ textAlign: "center" }}>
                Infelizmente não temos nenhuma organização que caiba no seu
                critério, procure na maior cidade mais próxima de você :(
              </p>
            ) : (
              <Pagination
